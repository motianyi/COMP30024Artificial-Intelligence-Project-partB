...
def heuristic2(hexes, exitedPieces, jumpColour):
    result = {'red':0, 'green':0, 'blue': 0}
    for colour in result.keys():
        result[colour] = h(colour, hexes, exitedPieces, jumpColour)
    return result
...
class ExamplePlayer:
    ...
    def action(self):
        ...
        """
        depth = 3
        return self.maxN(self.hexes.copy(),self.exitedPieces.copy(), depth, self.colour,None)[1]



    def maxN(self, hexes, numExits, depth, colour,jumpColour):
        # TODO, never goal state,
        if (depth == 0 or isGoalState(numExits, colour)):
            return (heuristic2(hexes,numExits,jumpColour),[('PASS', None)])
        ...
        if(len(succ) == 0):
            (valuesVector, action) = self.maxN(hexes.copy(), numExits.copy(), depth-1, nextPlayer(colour),jumpColour)
            return (valuesVector, ('PASS', None))
        for (succHexes, succNumExits, move, jumpColour) in succ:
            # print(move)
            (valuesVector, action) = self.maxN(succHexes.copy(), succNumExits.copy(), depth-1, nextPlayer(colour),jumpColour)
